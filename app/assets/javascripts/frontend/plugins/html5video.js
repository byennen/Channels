jQuery(function ($) {
    ...
    projekktor = $p = function () {
        ...
        function PPlayer(srcNode, cfg, onReady) {
            this.config = new projekktorConfig("1.0.22");
            ...
            this._prepareMedia = function (data) {
                ...
                var extRegEx = [];
                for (var i in $p.mmap) {
                    if (!this._canPlay($p.mmap[i].type, $p.mmap[i].platform, data.config.streamType || "http")) {
                        continue
                    }
                    $p.mmap[i].level = this.config._platforms.indexOf($p.mmap[i].platform);
            ...
            this._windowTouchListener = function (evt) {
                if (evt.touches) {
                    if (evt.touches.length > 0) {
                        if (($(document.elementFromPoint(evt.touches[0].pageX, evt.touches[0].pageY)).attr("id") || "").indexOf(this.getDC().attr("id")) == 0) {
            ...
            this._canPlay = function (type, platform, streamType) {
                var checkIn = [],
                    checkFor = [],
                    st = streamType || "http",
                    pltfrm = (platform != null) ? platform.toUpperCase() : null;
                if (this._compTableCache == false) {
                    this._compTableCache = this._testMediaSupport()
                }
                $.each(this._compTableCache[st] || [], function (key, val) {
                    if (pltfrm != null) {
                        if (key != pltfrm) {
                            return true
                        }
                ...
                var ref = this,
                    fullScreenApi = {
                        ...
                        isFullScreen: function () {
                            try {
                                return this.dest.hasClass("fullscreen")
                            } catch (e) {
                                return false
                            }
                        },
            ...
            this.setActiveItem = function (mixedData) {
                ...
                if (!$p.models[newModel]) {
                    ...
                    if (this.getConfig("className") != null) {
                        this.getDC().addClass(this.getConfig("cssClassPrefix") + this.getConfig("className"))
                    }
            ...
            this.reset = function () {
                ...
                var result = false;
                $.each(this.getCuePoints(idx), function () {
                    if (this.id == id) {
                        result = this
                    }
                });
            ...
            this._testMediaSupport = function () {
                ...
                for (var i = 0; i < $p.mmap.length; i++) {
                    platform = $p.mmap[i]["platform"].toUpperCase();
                    ...
                    $.each(streamType, function (key, st) {
                        ...
                        if (ref["get" + capped + "Version"]($p.mmap[i]["type"]) > 0) {
                            if (ref.getConfig("enable" + capped + "Platform") !== true) {
                                return true
                            }
...
jQuery(function (a) {
    projekktorPluginInterface.prototype = {
        ...
        applyToPlayer: function (e, d) {
            var b = this.getConfig("cssClassPrefix");
            if (!e) {
                return null
            }
        ...
        getElement: function (b) {
            return this.pp.env.playerDom.find("." + this.getConfig("cssClassPrefix") + b)
        },
...
projekktorConfig.prototype = {
    ...
    "_reelParser": null,
    "_cssClassPrefix": "pp",
    "_platforms": ["browser", "ios", "native", "flash"],
    "_iframe": false,
...
jQuery(function (a) {
    $p.newModel({
        ...
        applySrc: function () {
            ...
            a.each(d, function () {
                a(document.createElement("source")).appendTo(f.mediaElement).attr({
                    src: this.src,
                    type: this.type
                })
            });
            var e = function () {
                    f.removeListener("canplay", "qs");
                    f.removeListener("loadstart", "qs");
        ...
        playingListener: function (c) {
            ...
            (function () {
                try {
                    if (b.mediaElement[0].networkState == b.mediaElement[0].NETWORK_NO_SOURCE && b.getDuration() == 0) {
                        b.setTestcard(80);
                        return
                    }
                    if (b.getDuration() == 0) {
...
jQuery(function (a) {
    playerModel.prototype = {
        ...
        displayItem: function (b) {
            ...
            if (this.hasGUI) {
                this.pp.env.playerDom.children().not("." + this.pp.getConfig("cssClassPrefix") + "display").addClass("inactive").removeClass("active")
            }
        ...
        setTestcard: function (f, b) {
            ...
            this.mediaElement = a(document.createElement("div")).addClass(this.pp.getConfig("cssClassPrefix") + "testcard").attr("id", this.pp.getId() + "_testcard_media").appendTo(e);
            if (c.length > 0) {
    ...
    projekktorControlbar.prototype = {
        ...
        initialize: function () {
            var f = this,
                ...
                c = true,
                b = this.getConfig("cssClassPrefix");
        ...
        displayVolume: function (e) {
            ...
            vknob = this.cb.find("." + this.getConfig("cssClassPrefix") + "vknob"), vslider = this.cb.find("." + this.getConfig("cssClassPrefix") + "vslider");
            ...
            this.controlElements.vknob.css("left", e * 100 + "%").css("left", e * 100 + "%");
        ...
        displayCuePoints: function (c) {
            var b = this;
            if (!this.getConfig("showCuePoints")) {
                return
            }
            a.each(this.pp.getCuePoints() || [], function () {
                var d = Math.max((this.off - this.on) * 100 / c, Math.round(c / 100), 1),
                    g = (this.on * 100 / c) - ((d / 2) * 100 / c),
                    f = this,
                    e = b.pp,
                    h = a(document.createElement("div")).addClass(b.getConfig("cssClassPrefix") + "cuepoint").addClass("inactive").css("left", g + "%").css("width", d + "%");
                this.addListener("unlock", function () {
                    a(h).removeClass("inactive").addClass("active");
                    h.click(function () {
                        b.pp.setPlayhead(h.data("on"))
                    })
                });
                a(".ppscrubber").append(h)
            })
        },
